...
class TBOT_BDD:
    ...
    async def get_id_survivant(self, name: str):
        ...
        async with db.execute(
            f"""SELECT * FROM 'survivant' WHERE name_lower='{name.lower()}' """
        ) as cur:
    ...
    async def get_stats_survivant(self, id_twitch: int) -> dict:
        ...
        Returns:
            dict: renvois un dictionnaire contenant les
            statistiques du survivant.
        """
        reponse = {}
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        async with db.execute(
    ...
    async def stat_raid(self, id_twitch: int):
        ...
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        async with db.execute(
            f"""SELECT * FROM 'raid'  WHERE id_twitch='{id_twitch}' """
        ) as cur:
    ...
    async def calcul_ratio_raid(
        ...
        """Calcul du ratio reussite du RAID en fonction du type de raid
        et des attributs du survivant
        ...
        Returns:
        tuple: ensemble des valeurs definissant le RAID influencées par les
        stats du survivants
        ...
        await tbot_com.ecrit_log(
            f"Name>{raid_stat['name']} : Butin={BUTIN}%,\
                Bredouille={BREDOUILLE}%, Blesse={BLESSE}%,Mort={MORT}%,\
                Distance={DISTANCE}."
        )

        return (BUTIN, BREDOUILLE, BLESSE, MORT, DISTANCE)

    async def raid_initialise(
        self, survivant_stat: dict, type_raid: str, cout_raid: int
    ) -> None:
        DISTANCE = self.config_raid_json["raid_" + type_raid]["distance_raid"]
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        await db.execute(
            """INSERT OR IGNORE INTO raid
                        (id_twitch,
                        name,
                        type,
                        distance,
        ...
        await db.execute(
            f"""UPDATE survivant SET credit = credit - {cout_raid},
                         inraid = 1
    ...
    async def raid_generation(self, raid: dict) -> None:
        ...
        type_raid = "raid_" + raid["type"]
        DISTANCE_REF = self.config_raid_json[type_raid]["distance_raid"]
        BUTIN_REF = self.config_raid_json[type_raid]["stats_raid"]["niveau-1"]["BUTIN"]
        BREDOUILLE_REF = self.config_raid_json[type_raid]["stats_raid"]["niveau-1"]["BREDOUILLE"]
        BLESSE_REF = self.config_raid_json[type_raid]["stats_raid"]["niveau-1"]["BLESSE"]
        MORT_REF = self.config_raid_json[type_raid]["stats_raid"]["niveau-1"]["MORT"]
        gfx_car = f'{raid["levelRaid_transport"]}-'\
            + f'{(random.randrange(4)+1)}.png'
        embuscade = []
        ...
        for tentative_survie in range(tentative):
            ...
            if resultRAID < MORT:  # Mort sans appel
                ...
                fin = 0
                (composition_butin,
                 bonus_butin) = await self.genere_butin(raid)
                break
        if fin > 0:
        ...
        for i in range(random.randrange(1, 3)):
            while "Tant que l on ne trouve pas une bonne distance":
                proposition_ambush = random.randrange(DISTANCE - 50,
                                                      DISTANCE - 10)
                if (
                    str(proposition_ambush) not in embuscade
                    and proposition_ambush - 1 != blesse
                    and proposition_ambush - 1 != fin
                    and proposition_ambush - 1 > fin
                ):
    ...
    async def add_credit(self, id_twitch, credit):
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        await db.execute(
            f"""UPDATE survivant SET credit = credit+{credit} \
                WHERE id_twitch = {id_twitch}"""
        )
    ...
    async def genere_butin(self, raid: dict) -> tuple:
        ...
        Returns:
            tuple : (dict > dict avec nom et class du loot,
                     int -> bonus_butin).
        """
        ...
        for i in range(
            raid["levelRaid_gear"]
        ):  # Autant de tour que le niveau d'equipement
            hasard = random.randrange(100)
            object_tier1 = self.config_ratio_json[f"objet_{i+1}"][
                f"armement_{raid['levelRaid_weapon']}"
            ]["tier_1"]
            object_tier2 = self.config_ratio_json[f"objet_{i+1}"][
                f"armement_{raid['levelRaid_weapon']}"
            ]["tier_2"]
            object_tier3 = self.config_ratio_json[f"objet_{i+1}"][
                f"armement_{raid['levelRaid_weapon']}"
            ]["tier_3"]
            ...
            if (
                random.randrange(100) >= CONFIG["GEAR_BOOST"]
            ):  # test si on arrete le tour (100-CONFIG["GEAR_BOOST"] que oui)
                break
    ...
    async def actualise_statRaid(self, channel):
        ...
        for raid_actif in listeRaid:
            ...
            if raid["time_renfort"] > CONFIG["MAX_TIME_RENFORT"]:
                DISTANCE_POURCENT = (
                    raid["distance"] * 100) // (raid["michemin"] * 2
                                                )
                raid["distance"] -= 1
            else:
            ...
            await db.execute(
                f"""UPDATE raid SET distance = {raid["distance"]},\
                    time_renfort = time_renfort+1 \
                    WHERE id_twitch = {raid["id_twitch"]}"""
            )

            if raid["time_visi"] > 0:
                await db.execute(
                    f"""UPDATE raid SET time_visi = time_visi - 1 \
                        WHERE id_twitch = {raid["id_twitch"]}"""
                )
            if raid["time_visi"] == 0 and raid["visi"] is True:
                await db.execute(
                    f"""UPDATE raid SET visi = False \
                        WHERE id_twitch = {raid["id_twitch"]}"""
                )

            # affiche toujours les infos raid si moins de 5 raids
            # en simultané
            if (
                len(listeRaid) < CONFIG["MAX_VISI"]
                or raid["time_renfort"] < CONFIG["MAX_TIME_RENFORT"]
            ):
                raid["visi"] = True
            ...
            if str(raid["distance"]) in embuscade:
                ...
                await tbot_com.message(
                    "raid_embuscade", channel=channel, name=raid["name"]
                )

            data[f'SURVIVANT_{raid["name"]}'] = {
                "NAME": raid["name"],
            ...
            if raid["distance"] <= 0:
                if raid["mort"] > 1:
                    ...
                    await db.execute(
                        f"""UPDATE survivant SET alive = 0,inraid = 0 \
                            WHERE id_twitch = {raid["id_twitch"]}"""
                    )
                else:
                    await self.gere_fin_raid(db, raid, channel)
                    ...
                    await db.execute(
                        f"""UPDATE survivant SET inraid = 0 \
                            WHERE id_twitch = {raid["id_twitch"]}"""
                    )
                ...
                await tbot_com.message(
                    "raid_blesse", channel=channel, name=raid["name"]
                )
        ...
        async with aiofiles.open(
            "TBoT_Overlay/raid.json", "w", encoding="utf-8"
        ) as fichier:
    ...
    async def gere_fin_raid(self, db, raid, channel):
        ...
        if raid["resultat"] == "BUTIN":
            ...
            listebutin = "<br>" + await tbot_com.donne_butin(
                raid["composition_butin"])
            await tbot_com.message(
                "raid_win_butin",
                channel=channel,
                name=raid["name"],
                gain_prestige=str(gain_prestige),
                listebutin=listebutin,
                bonus_butin=str(raid["bonus_butin"]),
            )
            await db.execute(
                f"""UPDATE survivant SET prestige = \
                    prestige +{gain_prestige+raid['bonus_butin']} \
                    WHERE id_twitch = {raid['id_twitch']}"""
            )
        if raid["resultat"] == "BREDOUILLE":
            ...
            await db.execute(
                f"""UPDATE survivant SET prestige = prestige +{gain_prestige} \
                    WHERE id_twitch = {raid['id_twitch']}"""
            )
        ...
        for joueur in liste:
            if joueur != "":
                # todo: gerer les nom des support avec
                # les id twitch
                await db.execute(
                    f'''UPDATE survivant \
                        SET prestige = prestige +{gain_prestige},\
                                 support_raid = "",\
                                 inraid       = False\
    ...
    async def upgrade_aptitude(self, id_twitch: int,
                               aptitude: str,
                               cout_upgrade: int):
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        await db.execute(
            f"""UPDATE survivant SET prestige = prestige - {cout_upgrade} \
                WHERE id_twitch = {id_twitch}"""
        )
        await db.execute(
            f"""UPDATE survivant SET level_{aptitude} = level_{aptitude} + 1 \
                WHERE id_twitch = {id_twitch}"""
        )
        await db.commit()
        await db.close()
    ...
    async def add_visi(self, id_twitch: str):
        ...
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        await db.execute(
            f"""UPDATE raid SET visi=true,
                                time_visi = {CONFIG["TIME_VISI"]} \
                WHERE id_twitch = {id_twitch}"""
        )
        await db.commit()
        await db.close()
    ...
    async def support_revision(self,
                               type: str,
                               raider: dict,
                               helper_stats: dict):
        ...
        if scoreHaF is None:
            ...
            await db.execute(
                """INSERT OR IGNORE INTO HallOfFame
                        ...
                        score)
                        VALUES (?,?,?)""",
                (id_twitch, survivant["name"], score),
            )
            await db.commit()
            await db.close()
            await tbot_com.message(
                ...
                channel=channel,
                name=survivant["name"],
                gain_prestige=str(score),
            )

        elif scoreHaF[3] < score:
            db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
            await db.execute(
                f"""UPDATE HallOfFame SET score = {score}
                              WHERE id_twitch = {id_twitch}"""
            )
            await db.commit()
            await db.close()
    ...
    async def Get_TopScore(self):
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        async with db.execute(
            """SELECT * FROM 'HallOfFame' ORDER BY score DESC LIMIT 10"""
        ) as cur:
    ...
    async def init_score(self):
        db = await aiosqlite.connect(os.path.join("./Sqlite", self.NAMEBDD))
        async with db.execute(
            """SELECT * FROM 'HallOfFame' ORDER BY score DESC LIMIT 5"""
        ) as cur:
            liste = await cur.fetchall()
        await db.close()
        cout_prestige = CONFIG["TARIF_UPGRADE"]
