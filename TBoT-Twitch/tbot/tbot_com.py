...
async def msg_init_TboT():
    for i in range(30):
        ligne_overlay.append({"date": 0, "message": ""})
...
async def message(
    ...
    ovl = (
        ovl.replace("{name}", f"<span class='pseudo'>{name}</span>")
        .replace("{name2}", name2)
        .replace("{heure}", heure)
        .replace("{minute}", minute)
        .replace("{gain_prestige}", gain_prestige)
    ...
    if mod != "":
        async with aiofiles.open(
            URLMOD + os.sep + "radio.txt", "w", encoding="utf-8"
        ) as fichier:
...
async def donne_butin(butin: str) -> str:
    ...
    async with aiofiles.open(
        URLMOD + os.sep + "butin.txt", "w", encoding="utf-8"
    ) as fichier:
